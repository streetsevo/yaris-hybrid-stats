...
Разверни на Streamlit Community Cloud: положи этот файл, requirements.txt
и hybridassistant.db в один репозиторий на GitHub.
...
DB_PATH = "hybridassistant.db"
MAINTENANCE_FILE = "maintenance.json"

# --- Защита формы добавления записей ТО ---
# Пароль НИКОГДА не хранится в коде в открытом виде (код лежит в
# публичном репозитории GitHub вместе с базой данных!). Вместо этого
# хранится и сравнивается только SHA-256 хеш пароля.
#
...
TR = {
    "ru": {
        ...
        "db_missing": "⚠️ Пожалуйста, загрузите базу данных в репозиторий (файл `hybridassistant.db` не найден или повреждён).",
    ...
    "pl": {
        ...
        "db_missing": "⚠️ Proszę wgrać bazę danych do repozytorium (plik `hybridassistant.db` nie został znaleziony lub jest uszkodzony).",
...
@st.cache_data(show_spinner=False)
def load_trips_with_consumption(db_path: str) -> pd.DataFrame:
    ...
    if not _table_exists(db_path, "TRIPS"):
        return pd.DataFrame()
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        fastlog_available = _table_exists(db_path, "FASTLOG")
        fuel = (
            pd.read_sql_query(
...
@st.cache_data(show_spinner=False)
def load_cell_delta_series(db_path: str) -> pd.DataFrame:
    ...
    if _table_exists(db_path, "BATTLOG"):
        with sqlite3.connect(db_path) as conn:
    ...
    if _table_exists(db_path, "HVCHECKCELL"):
        with sqlite3.connect(db_path) as conn:
...
def _expected_password_hash() -> str:
    """Хеш пароля берётся из st.secrets, если он там задан (рекомендуемый
    способ для продакшена — тогда пароль не попадает в git-репозиторий),
    иначе используется запасной хеш, зашитый в код."""
    try:
        return st.secrets.get("maintenance_password_hash", _FALLBACK_PASSWORD_HASH)
    except Exception:
        return _FALLBACK_PASSWORD_HASH


def _verify_password(password_input: str) -> bool:
    """Сравнение хешей за постоянное время (защита от timing-атак),
    пароль в открытом виде никогда не хранится и не логируется."""
    input_hash = hashlib.sha256(password_input.encode("utf-8")).hexdigest()
    return hmac.compare_digest(input_hash, _expected_password_hash())


def _lockout_remaining_seconds() -> int:
    """Сколько секунд ещё осталось до снятия блокировки (0, если блокировки нет)."""
    lockout_until = st.session_state.get("maintenance_lockout_until", 0.0)
    remaining = lockout_until - time.time()
    return max(0, int(remaining))


def _register_failed_attempt() -> None:
    attempts = st.session_state.get("maintenance_failed_attempts", 0) + 1
    st.session_state["maintenance_failed_attempts"] = attempts
    if attempts >= MAX_PASSWORD_ATTEMPTS:
        st.session_state["maintenance_lockout_until"] = time.time() + LOCKOUT_SECONDS
        st.session_state["maintenance_failed_attempts"] = 0


def _register_successful_unlock() -> None:
    st.session_state["maintenance_unlocked"] = True
    st.session_state["maintenance_failed_attempts"] = 0
    st.session_state["maintenance_lockout_until"] = 0.0
...
def render_maintenance_tab():
    ...
    if not st.session_state.get("maintenance_unlocked", False):
        ...
        elif _verify_password(password_input):
            _register_successful_unlock()
            st.rerun()
        else:
            _register_failed_attempt()
            attempts_left = MAX_PASSWORD_ATTEMPTS - st.session_state.get(
                "maintenance_failed_attempts", 0
            )
            st.error(t("password_wrong").format(attempts_left=max(attempts_left, 0)))
        return
...
def main():
    ...
    if not os.path.exists(DB_PATH):
        ...
        try:
            trips_df = load_trips_with_consumption(DB_PATH)
            temp_df = load_temperature_log(DB_PATH)
            cell_df = load_cell_delta_series(DB_PATH)
